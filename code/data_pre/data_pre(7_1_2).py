...
OUTPUT_DIR = ROOT_DIR / "data_pre" / "data_pre_outputs(7_1_2)"
...
# =========================
# 划分比例：训练 70% / 验证 10% / 测试 20%（7:1:2）。
# 相比 7:2:1，测试集扩大到约一整年，保证年度评估有效性（与研究进展讨论反馈一致）。
TRAIN_RATIO = 0.7
VAL_RATIO = 0.1
...
def split_by_time(df, train_ratio=TRAIN_RATIO, val_ratio=VAL_RATIO):
    n_rows = len(df)
...
# =========================
# 4. Train / validation / test split (7:1:2)
# =========================
train_df, val_df, test_df = split_by_time(df, train_ratio=TRAIN_RATIO, val_ratio=VAL_RATIO)
...
train_df.to_csv(MODEL_DATA_DIR / "train_model_input.csv", index=False, encoding="utf-8-sig")
val_df.to_csv(MODEL_DATA_DIR / "val_model_input.csv", index=False, encoding="utf-8-sig")
test_df.to_csv(MODEL_DATA_DIR / "test_model_input.csv", index=False, encoding="utf-8-sig")

# Legacy filenames kept for backward compatibility with older base_model scripts.
...
print("Data preprocessing completed (7:1:2).")
