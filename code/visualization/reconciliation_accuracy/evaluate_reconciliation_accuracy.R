...
method_order <- c("Base", "BU", "OLS", "WLS", "DynamicWLS")

# 柱子填充色参考 multi_seed_boxplots.R 的柔和配色；前三个颜色沿用参考图，
# 后两个颜色选择同样低饱和度的绿和金色，保证五种方法能区分但不过分刺眼。
...
figure_name <- "fig_reconciliation_accuracy_metrics" # 输出文件名主干；脚本会同时保存 PNG 和 PDF。
...
make_accuracy_plot <- function(plot_data) {
  ...
  # 只在 KGE 和 NSE 面板绘制 y = 0 参考线；使用线段而不是 geom_hline，
  # 避免不同 ggplot2 版本对 geom_hline 参数兼容性不同而产生警告。
...
main <- function() {
  ...
  # 同一张图同时导出 PNG 和 PDF：PNG 便于预览和插入文档，PDF 保留矢量信息便于论文排版。
