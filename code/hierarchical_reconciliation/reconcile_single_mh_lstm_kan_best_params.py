...
from base_model.multi_output_model.models import HORIZONS, KAN, TARGET_MAP_RAW, USE_LOG, build_model  # noqa: E402
...
def load_target_outputs(target, train_df, val_df, test_df, date_col, device):
    ...
    for split in ["val", "test"]:
        ...
        true = data[split]["y"]
        pred_raw = np.stack([INVERSE_FN(pred[:, [h]], data["y_scaler"])[:, 0] for h in range(len(HORIZONS))], axis=1)
        true_raw = np.stack([INVERSE_FN(true[:, [h]], data["y_scaler"])[:, 0] for h in range(len(HORIZONS))], axis=1)
